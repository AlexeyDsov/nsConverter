...
class AllCommand
{
	...
	private function parseJson($path)
	{
		...
		if ($jsonError) {
			throw new ConfigReadException("Json parse error {$jsonError}\n");
		}
		throw new ConfigReadException("empty or incorrect json in --config\n");
	}

	private function getAction(array $cmdParams = [])
	{
		if (!isset($cmdParams['-Daction'])) {
			throw new ConfigReadException("-Daction=[scan|replace] not found in arguments");
		} elseif (!in_array($action = $cmdParams['-Daction'], ['scan', 'replace'])) {
			throw new ConfigReadException("-Daction=[scan|replace] wrong value");
		}
		return $action;
	}
	...
	{
		if (!file_exists($path)) {
			throw new ConfigReadException("config path does not exists: ".$path);
		} elseif (!is_readable($path)) {
			throw new ConfigReadException("config path not readable: ".$path);
		} elseif (!is_file($path)) {
			throw new ConfigReadException("config path not a file: ".$path);
		}
	}
}
